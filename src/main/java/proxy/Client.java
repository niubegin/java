...
public class Client {
     ...
     * 可以看到，我们可以通过LogHandler动态代理不同类型的对象，如果我们把对外的接口都通过动态代理来实现，
     * 那么所有的函数调用最终都会经过invoke函数做转发，因此我们就可以在这里做一些自己想做的操作，
     * 比如日志系统、事务、拦截器、权限控制等。这也就是AOP(面向切面编程)的基本原理。
     * AOP（Aspect Oriented Programming）：将日志记录，性能统计，安全控制，事务处理，异常处理等代码从业务逻辑代码中划分出来，
     * 通过对这些行为的分离，我们希望可以将它们独立到非指导业务逻辑的方法中，进而改变这些行为的时候不影响业务逻辑的代码---解耦。
     * 针对如上的示例解释：
     * 我们来看上面的UserManagerImplProxy类，它的两个方法System.out.println("start-->addUser()")和System.out.println("success-->addUser()")，
     * 这是做核心动作之前和之后的两个截取段，正是这两个截取段，却是我们AOP的基础，在OOP里，
     * System.out.println("start-->addUser()")、核心动作、System.out.println("success-->addUser()")
     * 这三个动作在多个类里始终在一起，但他们所要完成的逻辑却是不同的，
     * 如System.out.println("start-->addUser()")里做的可能是权限的判断，在所有类中它都是做权限判断，而在每个类里核心动作却各不相同，
     * System.out.println("success-->addUser()")可能做的是日志，在所有类里它都做日志。
     * 正是因为在所有的类里，核心代码之前的操作和核心代码之后的操作都做的是同样的逻辑，因此我们需要将它们提取出来，单独分析，设计和编码，
     ...
     * 使其复用性和易用性得到进一步提升而这不仅仅符合了面向对象的设计理念，其中还有AOP的身影，这也提供给我们对类抽象的一种参考。
